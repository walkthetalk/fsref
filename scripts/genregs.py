...
class VMFsctl(VerilogModuleFile):
	...
	def saveCustom(self, lvl):
		...
		ret += suppline(lvl, 'reg clk_toggle;')
		ret += suppline(lvl, 'always @ (posedge clk) begin')
		ret += suppline(lvl+1, 'if (resetn == 0)')
		ret += suppline(lvl+2, 'clk_toggle <= 0;')
		ret += suppline(lvl+1, 'else')
		ret += suppline(lvl+2, 'clk_toggle <= ~clk_toggle;')
		ret += suppline(lvl, 'end')
		...
		ret += suppline(lvl+2, 'wre_sync[i]     <= ((clk_d2 != clk_d1) && (wr_en_d2 && o_resetn) && (wr_addr_d2 == i));')
