...
pip_add_bus_if $core I_AXIS {
	abstraction_type_vlnv {xilinx.com:interface:axis_rtl:1.0}
	bus_type_vlnv {xilinx.com:interface:axis:1.0}
	interface_mode {slave}
} {
	TVALID	si_axis_tvalid
	TDATA	si_axis_tdata
	TUSER	si_axis_tuser
	TLAST	si_axis_tlast
	TREADY	si_axis_tready
}
append_associate_busif clk I_AXIS

pip_add_bus_if $core I_AXIS_INDEX [subst {
	abstraction_type_vlnv {$VENDOR:interface:window_ctl_rtl:1.0}
	bus_type_vlnv {$VENDOR:interface:window_ctl:1.0}
	interface_mode {slave}
}] {
	WIDTH   si_axis_source_x
	HEIGHT  si_axis_source_y
}
append_associate_busif clk I_AXIS_INDEX
...
pip_add_bus_if $core s_axis_resetn {
	abstraction_type_vlnv xilinx.com:signal:reset_rtl:1.0
	bus_type_vlnv xilinx.com:signal:reset:1.0
	interface_mode slave
} {
...
pip_add_bus_if $core clk {
	...
	ASSOCIATED_RESET {resetn:en_overlay:s_axis_resetn}
}]
...
pip_add_usr_par $core {C_S_CHANNEL} {
	display_name {Input Channel Number}
	tooltip {Input Channel Number}
	widget {comboBox}
} {
	value_resolve_type user
	value 1
	value_format long
	value_validation_type list
	value_validation_list {0 1 3}
} {
	value 1
	value_format long
}

pip_add_usr_par $core {C_CHANNEL_WIDTH} {
	display_name {Input Channel Width}
	tooltip {Input Channel Width}
	widget {comboBox}
} {
	value_resolve_type user
	value 8
	value_format long
	value_validation_type list
	value_validation_list {8 10 12 16}
	enablement_tcl_expr {$C_S_CHANNEL > 0}
} {
	value 8
	value_format long
}
...
pip_add_usr_par $core {C_OUT_DW} {
	...
	value_resolve_type user
	enablement_value false
	value_tcl_expr {$C_CHANNEL_WIDTH * 3}
	value 24
	value_format long
} {
	value 24
